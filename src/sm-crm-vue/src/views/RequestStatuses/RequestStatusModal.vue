<script setup>
    ...
    function saveItem() {
        ...
        if (item.id > 0) {
            axios.put("/RequestStatuses/" + item.id, item).then(res => {
                toastr.success("RequestStatus updated!", "Updated")
                emit("onSaved", item)
            })
        } else {
            axios.post("/RequestStatuses", item).then(res => {
                toastr.success("RequestStatus created!", "Created")
                emit("onSaved", item)
            })
        }
    }
